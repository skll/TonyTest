(function($) {
    ...
    (function($) {
        $.fn.jqueryzoom = function(options) {
            ...
            $(this).hover(function() {
                ...
                if ($("div.jqueryzoomdiv").get().length == 0) {
                    $(this).after("<div class='jqueryzoomdiv'><img class='bigimg' src='" + bigimage + "'/></div>");
                    $(this).append("<div class='jqueryzoomPup'>&nbsp;</div>");
                }
                ...
                $("div.jqueryzoomdiv").css({
                    top: imageTop,
                    left: leftpos
                });
                $("div.jqueryzoomdiv").width(settings.xzoom);
                $("div.jqueryzoomdiv").height(settings.yzoom);
                $("div.jqueryzoomdiv").show();
                if (!settings.lens) {
                ...
                $(document.body).mousemove(function(e) {
                    ...
                    if (isNaN(scalex) | isNaN(scaley)) {
                        ...
                        if (((settings.xzoom) / (scalex * 1)) > bigwidth) {
                            $("div.jqueryzoomPup").width(bigwidth);
                        } else {
                            $("div.jqueryzoomPup").width((settings.xzoom) / (scalex * 1));
                        }
                        if (((settings.yzoom) / (scaley * 1)) > imageHeight) {
                            $("div.jqueryzoomPup").height(imageHeight);
                        } else {
                            $("div.jqueryzoomPup").height((settings.yzoom) / (scaley * 1));
                        }
                        if (settings.lens) {
                            $("div.jqueryzoomPup").css('visibility', 'visible');
                        }
                    }
                    xpos = mouse.x - $("div.jqueryzoomPup").width() / 2 - imageLeft;
                    ypos = mouse.y - $("div.jqueryzoomPup").height() / 2 - imageTop;
                    if (settings.lens) {
                        xpos = (mouse.x - $("div.jqueryzoomPup").width() / 2 < imageLeft) ? 0 : (mouse.x + $("div.jqueryzoomPup").width() / 2 > imageWidth + imageLeft) ? (imageWidth - $("div.jqueryzoomPup").width()) : xpos;
                        ypos = (mouse.y - $("div.jqueryzoomPup").height() / 2 < imageTop) ? 0 : (mouse.y + $("div.jqueryzoomPup").height() / 2 > imageHeight + imageTop) ? (imageHeight - $("div.jqueryzoomPup").height()) : ypos;
                    }
                    if (settings.lens) {
                        $("div.jqueryzoomPup").css({
                            top: ypos,
                            left: xpos
                        });
                    }
                    scrolly = ypos;
                    $("div.jqueryzoomdiv").get(0).scrollTop = scrolly * scaley;
                    scrollx = xpos;
                    $("div.jqueryzoomdiv").get(0).scrollLeft = (scrollx) * scalex;
                });
            },
        function() {
            ...
            if (settings.lens) {
                $("div.jqueryzoomPup").remove();
            }
            $("div.jqueryzoomdiv").remove();
        });
            count = 0;
            if (settings.preload) {
    ...
    function mycarousel_initCallback(carousel) {
        $("#mycarousel li").mouseover(function() {
            if ($("#mycarousel li").length > 1) {
                ...
                $("#_middleImage").css("padding", "0px");
                $("#_middleImage").show();
                $("#_middleImage").attr("src", ShowRightImgType(image_name, "minor")).attr("longdesc", ShowRightImgType(image_name, "max"));
    ...
    function ShowRightImgType(src, type) {
        src = src.replace(".minor.jpg", "").replace(".small.jpg", "");
        if (type == "small") {
            src = src + ".small.jpg";
        } else if (type == "minor") {
            src = src + ".minor.jpg";
        ...
        if (image.width > 0 && image.height > 0) {
            if (image.width / image.height >= ImgW / ImgH) {
                if (image.width > ImgW) {
                    ...
                    ImgD.style.padding = (ImgH - ImgD.height) / 2 + "px 0px";
                } else {
                ...
                if (ImgD.width < ImgW) {
                    ImgD.style.padding = "0px " + (ImgW - ImgD.width) / 2 + "px";
                }
            }
        }
    }
    $(function() {
        $(".tab_block a").click(function() {
            $(this).addClass("click").siblings().removeClass("click");
            var index = $(".tab_block a").index(this);
            $(".tab_info .right-center").eq(index).show().siblings().hide();
        });
    });
    function copy() {
